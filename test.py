...
def getwechatid(number, filepath):
    if os.path.getsize(filepath):
        return
    idlist = readwechatid(r'oldid.txt')
    worklist = []
    for i in range(number):
        idnumber = idlist.pop()
        worklist.append(idnumber)
    with open('oldid.txt', 'w+', encoding='utf-8') as file:
        file.truncate(0)
        for i in idlist:
            file.write(i + '\n')
...
#     return numbers
#
# file1_numbers = read_file('tt.txt')
# file2_numbers = read_file('WeChatId.txt')
#
# unique_to_file1 = set(file1_numbers) - set(file2_numbers)
# unique_to_file2 = set(file2_numbers) - set(file1_numbers)
#
# print('文件1中独有的号码：')
# for number in unique_to_file1:
#     print(f'{number} (来自文件1)')
#
# print('文件2中独有的号码：')
# for number in unique_to_file2:
#     print(f'{number} (来自文件2)')
